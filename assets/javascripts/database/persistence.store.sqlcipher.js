...
var persistence = (window && window.persistence_sqlcipher) ? window.persistence_sqlcipher : initPersistence(window.persistence_sqlcipher || {});
;
 ...
 * @param iOSLocation
 */
persistence.store.sqlcipher.config = function (persistence, dbname, password, dbversion, description, size, backgroundProcessing, iOSLocation) {
  var conn = null;
   ...
   */
  persistence.transaction = function (callback, error, success) {
    if (!conn) {
      throw new Error("No ongoing database connection, please connect first.");
    } else {
      conn.transaction(callback, error, success);
    }
  };
   ...
   * @returns {{}}
   */
  persistence.db.sqlcipher.connect = function (dbname, password, backgroundProcessing, iOSLocation) {
    var that = {};
    var conn = window.sqlitePlugin.openDatabase({name: dbname, key: password, location: (iOSLocation || 0)});
    native_conn = conn
    that.transaction = function (fn, error, success) {
      return conn.transaction(function (sqlt) {
        return fn(persistence.db.sqlcipher.transaction(sqlt));
      }, error, success);
    };
    return that;
  };
   ...
   * @returns {{}}
   */
  persistence.db.sqlcipher.transaction = function (t) {
    var that = {};
   ...
   * @returns {*}
   */
  persistence.db.connect = function (dbname, password, dbversion, description, size, backgroundProcessing, iOSLocation) {
    return persistence.db.sqlcipher.connect(dbname, password, backgroundProcessing, iOSLocation);
   ...
   */
  persistence.store.sqlcipher.sqliteDialect = {
  ...
  persistence.store.sql.config(persistence, persistence.store.sqlcipher.sqliteDialect);

  // Make the connection
  conn = persistence.db.connect(dbname, password, dbversion, description, size, backgroundProcessing, iOSLocation);
  if (!conn) {
...
var setupPeristenceSqlCMigrations = function(){
  var Migrator = {
    migrations: [],
    
    version: function(callback) {
      persistence_sqlcipher.transaction(function(t){
        t.executeSql('SELECT current_version FROM schema_version', null, function(result){
          if (result.length == 0) {
            t.executeSql('INSERT INTO schema_version VALUES (0)', null, function(){
              callback(0);
  ...
  Migration.prototype.executeActions = function(callback, customVersion) {
    ...
    persistence_sqlcipher.transaction(function(tx){
      function nextAction() {
  ...
  Migration.prototype.removeColumn = function(tableName, columnName) {
    this.action(function(tx, nextCommand){
      ...
      tx.executeSql(sql, null, function(result){
        ...
        persistence_sqlcipher.executeQueriesSeq(tx, queries, nextCommand);
      });
    });
  }
  ...
  persistence_sqlcipher.migrations = {};
  persistence_sqlcipher.migrations.Migrator = Migrator;
  persistence_sqlcipher.migrations.Migration = Migration;
  persistence_sqlcipher.migrations.init = function() { Migrator.setup.apply(Migrator, Array.prototype.slice.call(arguments, 0))};

  persistence_sqlcipher.migrate = function() { Migrator.migrate.apply(Migrator, Array.prototype.slice.call(arguments, 0))};
  persistence_sqlcipher.defineMigration = function() { Migrator.migration.apply(Migrator, Array.prototype.slice.call(arguments, 0))};

}


window.persistence_sqlcipher = persistence;
setupPeristenceSqlCMigrations()